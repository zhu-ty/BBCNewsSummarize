...
tf.app.flags.DEFINE_integer('eval_interval_secs', 3, 'How often to run eval.')
...
def _Train(model, data_batcher):
  ...
  with tf.device('/cpu:0'):
    ...
    sess = sv.prepare_or_wait_for_session(config=tf.ConfigProto(
        device_count={"CPU":50},allow_soft_placement=True))
    running_avg_loss = 0
    step = 0
...
def _Eval(model, data_batcher, vocab=None):
  ...
  sess = tf.Session(config=tf.ConfigProto(device_count={"CPU":50},allow_soft_placement=True))
  running_avg_loss = 0
  step = 0
  while True:
